# Meteo ELT Pipeline 🌤️

A fully automated, serverless ELT pipeline on AWS that ingests hourly weather data for multiple US cities, transforms it into analytics-ready tables, and runs daily without manual intervention.
...
### Automation
```
8:00am UTC  → EventBridge triggers Lambda → raw data lands in S3
...
### Extract
Python script calls the [Open-Meteo API](https://open-meteo.com/) — free, no API key required. Fetches hourly forecast data for 5 US cities:
- New York
- Los Angeles
- Chicago
- Houston
...
- **Source systems** → REST API ingestion
- **Storage** → S3 data lake with partitioning strategy
- **Ingestion** → batch ingestion pattern, scheduled daily
- **Transformation** → ELT with dbt, staging and mart layers
- **Serving** → Athena SQL for analytics
